...
function StatuteSection({
  ...
  return (
    <section className="space-y-2">
      ...
      <div className="rounded border border-[var(--border-color)]/50 bg-black/90 px-3 py-2.5 min-h-[4.5rem]">
        {text ? (
          <p className="text-xs text-[var(--foreground)]/60 leading-relaxed whitespace-pre-line">{text}</p>
        ) : (
          <p className="text-xs text-[var(--foreground)]/35 italic">{emptyHint}</p>
        )}
      </div>
    </section>
  );
...
export function MadoshoStatuteModal({ open, onClose, madoshoName, statute, atto }: Props) {
  ...
  const handleHeaderMouseDown = useCallback(
    (e: React.MouseEvent) => {
      if (!(e.target as HTMLElement).closest(".madosho-statute-drag-handle")) return;
  ...
  const statuteText = statute?.trim() ?? "";
  const attoText = atto?.trim() ?? "";
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="madosho-statute-title"
    >
      <div
      ...
      >
        <div className="madosho-statute-drag-handle flex items-center justify-between shrink-0 px-4 py-2.5 border-b border-[var(--border-color)] bg-black/40 select-none cursor-grab active:cursor-grabbing">
          <h3
            id="madosho-statute-title"
            className="font-display text-xs uppercase tracking-widest text-[var(--accent-gold)] flex items-center gap-2 pointer-events-none min-w-0"
          >
            <FontAwesomeIcon icon={icons.ordine} className="w-3.5 h-3.5 shrink-0" />
            <span className="truncate">{madoshoName}</span>
          </h3>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-5">
          <StatuteSection
            label="Statuto"
            text={statuteText}
            emptyHint="Nessuno statuto impostato."
          />
          <StatuteSection label="Atto" text={attoText} emptyHint="Nessun atto impostato." />
        </div>
      </div>
    </div>,
    document.body,
