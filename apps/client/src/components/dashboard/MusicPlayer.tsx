...
export function MusicPlayer() {
  ...
  return (
    <div
      className="mt-4 p-2.5 rounded-lg border border-[var(--accent-violet)]/20 flex flex-col gap-2"
      style={{
    ...
    >
      ...
      {/* COVER ART + TITOLO */}
      <div className="relative w-full h-[90px] rounded border border-white/10 overflow-hidden shrink-0">
        ...
        >
          <div className="absolute bottom-1 left-0 w-full text-center px-1.5 box-border">
            <div className="font-display text-[11px] text-[#e6e0ff] font-bold uppercase tracking-[1px] drop-shadow-[0_2px_4px_rgba(0,0,0,1)] whitespace-nowrap overflow-hidden text-ellipsis">
      ...
      <div className="flex justify-center items-center gap-3 pt-0.5">
        ...
        <button
          ...
          className="bg-transparent border-none cursor-pointer p-1 ml-2.5 flex items-center justify-center transition-opacity opacity-70 hover:opacity-100"
        >
          {isMuted ? (
      ...
      {playlists.length > 0 && (
        <select
          ...
          className="w-full py-1 px-2 bg-black/50 text-[#c9a84a] border border-white/10 rounded text-[10px] font-display cursor-pointer outline-none mt-0.5"
        >
          {playlists.map((p) => (
