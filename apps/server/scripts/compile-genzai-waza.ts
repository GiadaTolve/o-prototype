/**
 * Compila catalogo waza Genzai-dō sul database locale (DATABASE_URL).
 * Per Neon: bun run scripts/compile-genzai-waza.ts --neon
 * Singola waza: bun run scripts/compile-genzai-waza.ts -- --slug meisaku-opera-prima
 */
...
import {
  GENZAI_CATALOG,
  allGenzaiImplementazioneNotes,
  genzaiDescrizione,
  type GenzaiCatalogEntry,
} from "./genzai-catalog-data";

config({ path: resolve(import.meta.dir, "../../../.env") });

const useNeon = process.argv.includes("--neon");
const GENITORE = "Genzai-dō";
...
async function main() {
  ...
  const entries: GenzaiCatalogEntry[] = onlySlug
    ? GENZAI_CATALOG.filter((e) => e.slug === onlySlug)
    : GENZAI_CATALOG;

  if (onlySlug && entries.length === 0) {
    console.error(`Slug non in catalogo Genzai: ${onlySlug}`);
    process.exit(1);
  }
  ...
  try {
    for (const entry of entries) {
      ...
      const descrizione = genzaiDescrizione(entry);
    ...
    console.log(`\nOK ${entries.length} waza Genzai-dō → DB ${target}`);
    const notes = allGenzaiImplementazioneNotes();
    if (notes.length) {
