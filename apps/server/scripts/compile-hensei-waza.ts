/**
 * Compila catalogo waza Hensei-dō sul database locale (DATABASE_URL).
 * Per Neon: bun run scripts/compile-hensei-waza.ts --neon
 */
...
import {
  HENSEI_CATALOG,
  allHenseiImplementazioneNotes,
  henseiDescrizione,
} from "./hensei-catalog-data";

config({ path: resolve(import.meta.dir, "../../../.env") });

const useNeon = process.argv.includes("--neon");
const GENITORE = "Hensei-dō";
...
async function main() {
  ...
  try {
    for (const entry of HENSEI_CATALOG) {
      ...
      `;
      if (!row) {
        console.error(`MISSING slug: ${entry.slug} (genitore: ${GENITORE})`);
        process.exit(1);
      }
    ...
    console.log(`\nOK ${HENSEI_CATALOG.length} waza Hensei-dō → DB ${target}`);
    const notes = allHenseiImplementazioneNotes();
    if (notes.length) {
