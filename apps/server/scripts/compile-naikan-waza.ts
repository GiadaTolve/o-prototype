/**
 * Compila catalogo waza Naikan-dō sul database locale (DATABASE_URL).
 * Per Neon: bun run scripts/compile-naikan-waza.ts --neon
 */
...
import {
  NAIKAN_CATALOG,
  allNaikanImplementazioneNotes,
  naikanDescrizione,
} from "./naikan-catalog-data";

config({ path: resolve(import.meta.dir, "../../../.env") });

const useNeon = process.argv.includes("--neon");
const GENITORE = "Naikan-dō";
...
async function main() {
  ...
  try {
    for (const entry of NAIKAN_CATALOG) {
      ...
      const descrizione = naikanDescrizione(entry);
    ...
    console.log(`\nOK ${NAIKAN_CATALOG.length} waza Naikan-dō → DB ${target}`);
    const notes = allNaikanImplementazioneNotes();
    if (notes.length) {
